...
public class Unit{
    ...
    public void napadenie(Unit target){
        ...
        if (target.health>0 && health>0){
            ...
            va.damageArmor(); //повреждение защиты жертвы

            weapon.damageWeapon(); //повреждение оружия атакующего
    ...
    public int damagevictim(int a, double wp, double vp, int vi){
        return (int) Math.round(a*wp - vp*vi);
    }
